...
| `reviews` | `true` makes the agent's declared paths the change it judges rather than files it may change: it is handed those paths and their diff ahead of its task, claims nothing against other agents, and is stopped at its round cap with a request for its report. Only for a profile granting neither `write` nor `execute`. |
