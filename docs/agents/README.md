...
| `max_tokens` | Default token budget when the spawn names none, counted in new tokens — what the provider did not serve from its cache, plus the completion. Unset, the agent takes the ordinary default of 1200000, what one writer measured on one backlog item. A profile default is at least 300000; an explicit spawn may be as low as 200000 only when prompt admission still leaves its 200000-token working reserve. |
