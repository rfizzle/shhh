...
- **Storage is single-connection SQLite**: `SetMaxOpenConns(1)` is intentional. The WAL journal mode and busy timeout handle concurrency; don't open multiple `*DB` instances to the same file from one command. The one exception the code tolerates is the root's background history purge opening the store beside the command's own open, which is why `migrate` applies every step under `BEGIN IMMEDIATE` with the version re-read inside the lock — an `ALTER TABLE` applied twice fails the second opener.
