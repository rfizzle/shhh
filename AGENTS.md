...
one mechanism with three sources ([`docs/capabilities/subagents.md#three-can-steer-a-child-and-none-of-them-can-end-it`](docs/capabilities/subagents.md#three-can-steer-a-child-and-none-of-them-can-end-it)),
...
- **The deny mask is not configurable, and one tier of it is decided per session**: `fixedDenyPaths` (`internal/sandbox/sandbox.go`) cannot be disabled and `deny_extra` only adds to it — `~/.ssh`, `~/.aws`, `~/.config/gh`, `~/.netrc`, `~/.gnupg`, `~/.password-store`, `~/.secrets`, and shhh's own config and state dirs. `sandbox.CredentialPaths` (`~/.kube`, `~/.docker`, `~/.azure`, `~/.config/gcloud`, `~/.gem`) is the other tier: `resolvePolicy` masks each of those unless a write grant is at or inside it, so the working scope is what unmasks one and nothing subtracts from the mask. The order matters — the mask is assembled *after* the grants, and a grant below a store unmasks the store because masking the parent of a writable path is refused outright. `internal/scope` reads `sandbox.CredentialPaths` rather than keeping a second list: `Classify` returns Refused behind the fixed mask and Sensitive for a credential store, and two lists that had to agree would eventually grant a directory the mask still hides. A new store goes in the fixed list only if nothing legitimate ever writes to it; anything a session might work in goes in `CredentialPaths`, because the fixed list has no way back. See [`docs/capabilities/containment.md#the-deny-mask-is-not-configurable`](docs/capabilities/containment.md#the-deny-mask-is-not-configurable).
- **The temporary directory is the session's own, and the mechanism decides where it is**: `hostTempDirs` (`internal/sandbox/sandbox.go`) is `/tmp` plus whatever `TMPDIR` names when that is somewhere else, and `spec.privatiseTmp` hides both — it is the one part of the write boundary that used to be open in both directions, since `defaultWritePaths` granted `os.TempDir()` outright. `resolvePolicy` takes the mechanism for this and nothing else: bubblewrap mounts `--tmpfs` over each of them and `TMPDIR` becomes `/tmp`, Seatbelt cannot rearrange a filesystem so it denies them and `sessionTmpDir` hands out one directory per shhh process under the state dir — which the fixed mask already hides, so the profile allows it *after* the mask and SBPL's later-rule-wins is what makes one session's scratch its own. Nothing deletes a session directory when a session ends (there is no such seam — the wrap is built per command), so `sweepSessionTmpDirs` reaps the ones whose pid is gone on the way in, and leaves a name that is not a pid alone. **Order is the whole of how a grant still works**: the tmpfs is mounted before the write binds and the deny is written before the allowances, so `/add-dir /tmp/whatever` brings back exactly that path — and on Seatbelt a grant needs its *reads* allowed again too, because the deny took those and `(allow file-write*)` does not undo it (`tmpReadable`). `spec.tmpVisible` is the same re-exposure for a workspace or cwd that lives under `/tmp` without being a grant, which is every `t.TempDir()` in the suite and a read-only workspace under the quality gate. `withTmpdir` replaces the inherited `TMPDIR` rather than adding one, because `containedEnv` keeps the later pair and two of them is a coin toss. See [`docs/capabilities/containment.md#the-temporary-directory-is-the-sessions-own`](docs/capabilities/containment.md#the-temporary-directory-is-the-sessions-own).
