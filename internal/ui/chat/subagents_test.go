...
// regeneratedPatchAsk is a patch that touched a golden file, as the
// supervisor routes one: its generated file written by the generator.
func regeneratedPatchAsk(root string) *subagent.Ask {
	ask := longPatchAsk(root)
	ask.Files = append(ask.Files, "internal/agent/testdata/golden/loop.w80.txt")
	ask.Regenerated = []string{"go test ./internal/agent -update-golden"}
	return ask
}
...
// A patch whose generated files were regenerated names the generator that
// wrote them; one that touched none says nothing of the kind.
func TestChildAskPatchCardSaysWhatRegeneratedIt(t *testing.T) {
	dir := t.TempDir()
	view := ansi.Strip(routedModel(t, regeneratedPatchAsk(dir)).View().Content)
	if want := "by go test ./internal/agent -update-golden"; !strings.Contains(view, want) {
		t.Fatalf("the regenerated patch card should state %q:\n%s", want, view)
	}
	if plain := ansi.Strip(routedModel(t, longPatchAsk(dir)).View().Content); strings.Contains(plain, "generated by") {
		t.Fatalf("a patch with no generated files is not called generated:\n%s", plain)
	}
}
