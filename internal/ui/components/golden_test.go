...
// Golden-file render tests for the component catalog. The tests
...
// TestGolden_Anim captures the working label's motion: the
...
func goldenCatalog() []SelectOption {
	...
	// state why in the field at the end of them: a catalog you
...
// TestGolden_ReviewMode captures the review surface: the two
...
// TestGolden_FanoutBlock captures the fan-out block in the three
...
// uncommitted files, and the spinning pair carry the two shapes a steer
// leaves on a lane: one steered twice by its own reader and still off target,
...
// TestGolden_AttachmentChips captures the staged strip at every
...
// opens. What is typed is never rendered; the mask is.
...
// : the settings list, the picker that opens under the row being
...
// : the search on the left and the entry it selects on the
...
// : the fixed-width right-aligned columns, the one sparkline per
...
// : the grid row per check, the consequence stated in the words
...
// TestGolden_RateScreen captures `shhh rate`: one card holding both halves of
// the question, the three answers on the key row, and the row once there is
...
func TestGolden_RateScreen(t *testing.T) {
	captureGolden(t, "rate-screen", "the rating screen", goldenWidths, func(width int) []golden.Panel {
		...
		return []golden.Panel{
			{Label: "the question · what was asked, what came back, and what it exited",
				View: screen(nil).View(width)},
			{Label: "answered · the next card already, and this one never ran at all",
				View: func() string {
					r := screen(nil)
					r.Update(key("y"))
					return r.View(width)
