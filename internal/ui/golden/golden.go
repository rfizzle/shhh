...
// Cursor is where a surface put the terminal's own cursor, in the cells of
// the render it was taken from: column 0 is the render's first column and row
// 0 its first row.
//
// It is recorded because a surface that hands the terminal its cursor back
// leaves nothing in the render to show for it — the cell the cursor stands on
// is an ordinary space — so a cursor that drifted a column, or that stopped
// being placed at all, would be invisible to a capture that holds only
// characters.
type Cursor struct{ X, Y int }

// Case is what a golden records: which surface, rendered how wide, in which
// palette, where it left the cursor, and the panels themselves.
type Case struct {
	...
	Mono bool
	// Cursor is where the capture placed the terminal's cursor, or nil for a
	// render that places none — which is most of them, because a block
	// rendered on its own has no screen to place one on.
	Cursor *Cursor
	Panels []Panel
}
...
// cursorLabel is the header's reading of the cursor. "none" is a fact rather
// than an omission: a surface that used to place a cursor and stopped is a
// one-line diff, which a missing header line would not be.
func cursorLabel(c *Cursor) string {
	if c == nil {
		return "none"
	}
	return fmt.Sprintf("column %d, row %d", c.X, c.Y)
}
