...
// files, which it names: the writer's own bytes for one are never what lands,
...
func TestAKeptPatchLeavesItsGeneratedFilesToTheGenerator(t *testing.T) {
	...
	if len(c.kept.generated) != 1 || c.kept.generated[0] != "gen/out.txt" {
		t.Fatalf("the kept patch should name the generated file it left out, got %v", c.kept.generated)
	}
	if !strings.Contains(note, "gen/out.txt are generated and are regenerated when it lands") {
...
// A kept patch reviewed from its row lands with the generated files it was
// kept without regenerated over it, the card saying so first: the review is
// a landing like any other, and a landing never leaves a generated file at
// text its generator would not write.
func TestAKeptPatchLandsWithItsGeneratedFilesRegenerated(t *testing.T) {
	...
	if len(ask.Warnings) != 1 || !strings.Contains(ask.Warnings[0], "gen/out.txt are generated and are regenerated by their generator as this lands") {
		t.Fatalf("the card should say the generated file is regenerated as it lands, got %q", ask.Warnings)
	}
	ask.Respond(true)
	waitFor(t, func() bool { return !statusOf(t, sup, "writer-9").PatchKept })
	if take(repo, "a.txt") != "2" || take(repo, "gen/out.txt") != sumGolden("2", "1") {
		t.Fatalf("the source change should land with its golden regenerated, got %q", take(repo, "gen/out.txt"))
	}
	if !transcriptHas(sup.Transcript("writer-9"), EntrySystem, "gen/out.txt regenerated by gen sum") {
		t.Fatal("the landing note should say the generated file was regenerated")
	}
}
