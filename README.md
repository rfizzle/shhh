...
(`behavior.max_tool_rounds`, 25 by default). Reaching that used to end the turn
...
vitals rail carries the decision — `round 25/25 +10` while the offer stands,
`round 25/35` once it is taken.
...
There are two panes and one keyboard, and the surface always says which one has it. While the prompt has it the transcript hears no keys at all — a viewport handed every keystroke scrolls the history out from under the sentence being written, and the pager letters most terminal UIs bind (`j`, `k`, `u`, `d`, `f`, `b`, the spacebar) are exactly the letters a draft is made of. So the transcript is reached only by things a draft cannot produce: the mouse wheel, `PgUp`/`PgDn`, `Ctrl+E`, and `↑` on an empty prompt where the input history has nothing left to recall (where it has, `↑` stays the history's — that convention is older than this surface). The wheel scrolls the transcript, or the full-screen diff and review surfaces when those own the screen, and transfers nothing: the draft keeps the keyboard, so a scroll mid-sentence never swallows the next keystroke. Mouse reporting costs your terminal's own click-drag selection, which is a real trade — `/ui mouse off` gives it back and leaves the keyboard as the only way through the history.
...
| `/ui mouse <on\|off>` | Terminal mouse reporting. On (the default), the wheel scrolls the transcript; off, your terminal keeps its own click-drag selection |
