...
**Mouse reporting is off by default**, because it takes your terminal's own click-drag selection away, and a transcript is something you copy out of. Scrolling has substitutes here — `PgUp`/`PgDn`, `Shift+↑↓`, `Ctrl+E`, `j`/`k` — so it is not the wheel you are really deciding about; it is whether shhh or your terminal owns the mouse. Off is the safe default for a terminal you have not asked, and on is the better answer once you have (see below). (Off means off: shhh also asks your terminal to stop translating wheel notches into arrow keys, which is what most of them do to a full-screen program by default, and what used to make a flick of the wheel type into your draft.) `Ctrl+X` turns reporting on and off from anywhere, including while reading the transcript or inside the full-screen diff, and saves the answer to `appearance.mouse` so it holds for the next session too. `/ui mouse <on|off>` is the same setting said in words.
