...
**Mouse reporting is off by default**, because it costs your terminal's own click-drag selection and the two sides of that trade are not the same size: scrolling has substitutes here — `PgUp`/`PgDn`, `Shift+↑↓`, `Ctrl+E`, `j`/`k` — while selecting text has none, and a transcript is something you copy out of. (Off means off: shhh also asks your terminal to stop translating wheel notches into arrow keys, which is what most of them do to a full-screen program by default, and what used to make a flick of the wheel type into your draft.) `Ctrl+X` turns the wheel on and off from anywhere, including while reading the transcript or inside the full-screen diff, and saves the answer to `appearance.mouse` so it holds for the next session too. `/ui mouse <on|off>` is the same setting said in words.
