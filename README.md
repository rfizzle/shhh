...
| `appearance.mouse` | Terminal mouse reporting (default: false, so your terminal's own click-drag selection keeps working); on, the wheel scrolls, shhh's own click-drag selects the transcript, and a click opens the row or answers the approval key under it. `Ctrl+X` in a session flips and saves it |
